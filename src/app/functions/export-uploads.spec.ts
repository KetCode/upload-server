...
describe('export uploads', () => {
  ...
  it('should be able to export uploads', async () => {
    ...
    const generatedCSVStram = uploadStub.mock.calls[0][0].contentStream
    const csvString = await new Promise<string>((resolve, reject) => {
      const chunks: Buffer[] = []

      generatedCSVStram.on('data', (chunk: Buffer) => {
        chunks.push(chunk)
      })

      generatedCSVStram.on('end', () => {
        resolve(Buffer.concat(chunks).toString('utf-8'))
      })

      generatedCSVStram.on('error', err => {
        reject(err)
      })
    })

    const csvArray = csvString
      .trim()
      .split('\n')
      .map(row => row.split(','))
